...
def test_ensure_tables_creates_observations_table(db_engine):
    ...
    assert inspect(db_engine).has_table("fact_economic_observations", schema="raw")
...
def test_upsert_observations_inserts_all_rows(db_engine, sample_observations_df):
    ...
    with db_engine.connect() as conn:
        count = conn.execute(text("SELECT COUNT(*) FROM raw.fact_economic_observations")).scalar()
    assert count == 3
...
def test_upsert_dim_series_inserts_rows(db_engine, sample_dim_df):
    ...
    with db_engine.connect() as conn:
        count = conn.execute(text("SELECT COUNT(*) FROM raw.dim_series")).scalar()
    assert count == 3
